...
for this fork: the current design ("our approach" — self-contained
...
## Our approach: self-contained per-ruleset content scripts
...
## uBOL's approach: shared interpreter + storage-backed data
...
- **Highest-blast-radius change available to this project** — it touches
  how every ruleset injects on every page load, for every user,
  simultaneously, rather than being scoped to one subsystem.

## Why this fork uses our approach
...
3. **Our approach's disadvantages are genuinely minor**: some code
