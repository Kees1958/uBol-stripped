...
**v4 changes** (applying this document's own B4 to itself — what changed and why, not just that it did):
- **New B15** — consolidate near-duplicate parsers/handlers that differ only by a syntax marker; found via a real quote-splitting bug that existed purely because two copies of the same logic had quietly drifted.
- **New C25** — when the same bug class recurs across sibling features (three-plus times), the fix is a standing constraint doc entry *and* an automated check that scans for the pattern, not another one-off patch to the newest reported instance.
- **New C26** — an explicit structural/placement instruction ("between X and Y," a named UI landmark) is a literal instruction, not a category to reinterpret — ask one clarifying question up front if genuinely ambiguous, rather than substituting a plausible-sounding location and finding out it was wrong after the fact.
- **Part D** — check list extended to 15 items (was 12): scriptlet exception-rule logic, scripting site-mode gating, cosmetic exception-rule logic, all three verified against the real implementation rather than a hand-maintained re-check.
...
- [ ] A new feature that adds an exception/cancellation-rule syntax (per C21, ported from an existing proven one rather than reinvented) gets its own instance of step 13 added, exercising the real implementation per step 15's own requirement
