...
<img width="757" height="511" alt="image" src="https://github.com/user-attachments/assets/26d1198f-784f-43d2-909c-34719d1e0619" />


* Reason why AdGuard raw entries generate more DNR rules is that they are finer grained (DNR rules are hanlded by the browser and are processed very efficiently).
* Reason why uBol-stripped does not offer generic cosmetic filters (and only processes native procedural filters) is to compensate for the additional AsGuard scriplets
...
WHAT IS CHANGED?
In layman's terms we (Claude AI and me) used the strong bones of uBO-lite and stripped it from (less well programmed features to keep up with AdGuard Mv3). Next we added some open source AdGuard skills (it is open source so why not use what is available in stead of competing as a one man band show with a teal of developers). Next I added some features which in my opinion were missing in uBO-lite (see pictures). To make the code easy to understand, extend and debug I refactored the strong uBO-lite bones using four (old structured programming) principles (nerd alert): 
...
Because I added AdGuard skills (and uBO-lite is more programmed towards efficiency and performance than AdGuard Mv3) I wanted to regain the efficieny by using old and forgotten structured programming principles to optimize the code for performance, security and maintainability.  
