...
## Anti-tracking, Ads & Tracking Networks lists (Kees1958 + Disconnect + Ghostery)

Five separately-selectable static rulesets, per explicit request (each
individually toggleable in the Filter (block) lists panel, so a
filter-list issue can be narrowed to one specific source):

- **Kees1958 most-used EU+US ads & tracking networks**
  - **File:** `tools/data/kees1958-only-source.txt` (generated — see
    `tools/build-merged-adtracking-list.mjs`), compiled into
    `ruleset_kees1958`.
  - **Source:** Kees1958's own `EU_US_MV3_most_common_ad+tracking_
    ...
    not CC BY-NC-SA 4.0.
- **Disconnect ads & tracking networks**
  - **File:** `tools/data/disconnect-ads-source.txt`, compiled into
    `ruleset_disconnect_ads`.
  - **Source:** Disconnect's `services.json`, `Advertising` category —
    same CC BY-NC-SA 4.0 source already documented above in this file.
- **Ghostery ads & tracking networks**
  - **File:** `tools/data/ghostery-ads-source.txt`, compiled into
    `ruleset_ghostery_ads`.
  - **Source:** Ghostery TrackerDB, `advertising` category — same CC
    BY-NC-SA 4.0 source already documented above in this file.
- **Disconnect (Analytics, Cryptomining, Fingerprinting)**
  - **File:** `tools/data/disconnect-other-source.txt`, compiled into
    `ruleset_disconnect_other`.
  - **Source:** Disconnect's `services.json` — `Analytics`,
...
- **Ghostery (Analytics, Video players, Adult advertising)**
  - **File:** `tools/data/ghostery-other-source.txt`, compiled into
    `ruleset_ghostery_other`.
  - **Source:** Ghostery TrackerDB — `site_analytics`,
    `audio_video_player`, `pornvertising` categories, combined into one
    selectable list per explicit request.

All four Disconnect/Ghostery-sourced lists above: **License:** [Creative
Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC
BY-NC-SA 4.0)](https://creativecommons.org/licenses/by-nc-sa/4.0/) —
same terms as the Top-1M/Tracker-Domain Crossref entry above. Kees1958's
own list carries its own separate license terms, not CC BY-NC-SA 4.0.

**Safety filter, applied independently to each of the five:**
cross-referenced against `tools/data/top1m-tracker-crossref.json`'s
`reviewList` — any domain there (tracker-listed but NOT confirmed
advertising/analytics/social by either vendor, and a real top-1M
destination) is excluded from that specific list. See that file's own
entry above for what this filter is and why it exists. Splitting the
five lists apart (v9.1.1/v9.1.7) did not weaken this — the same
breakage-risk population is excluded from all five, independently.

**History:** originally one merged file (`ruleset_kees1958` alone,
"Merged Anti-tracking list (Kees1958 + Disconnect + Ghostery)") — split
into these five independently-selectable lists per explicit request, to
allow narrowing a filter-list issue down to one specific source.
