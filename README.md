...
Cookie banner removal only activates in Optimal mode (this is also different to uBO-lite which uses the same enabled filter is all modes). This two-approach prevents websites breakage (cookies are needed for login, so only move the slider on annoying websites to optimal).
